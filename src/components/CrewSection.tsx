...
import { useTeamMembers, useSiteContent } from "@/hooks/use-cms";
      ...
      <div className="relative glass-card rounded-xl overflow-hidden gradient-border glass-card-hover transition-all duration-500">
        <div className="bg-secondary/50 px-5 py-2.5 flex items-center justify-between border-b border-border/50">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono-terminal text-[10px] text-neon-green tracking-widest">CLEARANCE: {member.clearance_level}</span>
          </div>
        ...
        <div className="p-6 flex flex-col sm:flex-row gap-6">
          <div className="flex-shrink-0">
            <div className="w-24 h-24 rounded-lg border border-primary/20 overflow-hidden bg-secondary/30 flex items-center justify-center relative group-hover:border-primary/40 transition-colors">
              {member.avatar_url ? (
                <img src={member.avatar_url} alt={member.name} loading="lazy" width={96} height={96} className="w-full h-full object-cover" />
              ) : (
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              <span className="font-mono-terminal text-[10px] px-2.5 py-0.5 bg-primary/10 border border-primary/25 text-primary rounded-full">{member.alias}</span>
...
const CrewSection = () => {
  ...
  return (
    <section id="crew" ref={ref} className="relative py-28 px-4">
      ...
      <div className="relative max-w-4xl mx-auto">
        <motion.div style={{ y }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <span className="font-mono-terminal text-sm text-neon-red tracking-widest uppercase">{content?.crew_label || "// The Crew"}</span>
          <h2 className="font-display text-4xl md:text-6xl text-foreground mt-2">{content?.crew_title || "Operatives"}</h2>
          <p className="text-muted-foreground mt-4 max-w-lg mx-auto text-sm">{content?.crew_description || ""}</p>
